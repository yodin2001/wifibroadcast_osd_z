...
//RAPORT VARs
int stat_arm_var=0;
int curr_home_dist=0;
int rapo_home_dist=0;
int rapo_max_speed=0;
float curr_fly_time=0;
float start_point_alt=0;
float curr_c_alt=0;
float rapo_max_amp=0;
float rapo_max_alt=0;
float rapo_total_dist=0;
...
//-------------------------------------------------------------------------------------------------------------------------------------------nwo
 ...
 #endif
#if defined(VARIO) && defined(MAVLINK)
 draw_VARIO(td->mav_climb, VARIO_POS_X, VARIO_POS_Y, VARIO_SCALE * GLOBAL_SCALE);
 #endif
#if defined(SIDE_THR)
...
//-------------------------------------------------------------------------------------------------------------------------------------------nwo
 ...
 #endif
//#ifdef BASEALT 
        //draw_basealt(td->msl_altitude, BASEALT_POS_X, BASEALT_POS_Y, BASEALT_SCALE * GLOBAL_SCALE);
 //#endif
...
Fill(255,255,255,getOpacity(COLOR)); 
	Stroke(0,0,0,getOpacity(OUTLINECOLOR));
//	TextEnd(getWidth(94), getHeight(80), "ᐚ", osdicons, text_scale*2.3);	
	
	Fill(255,20,20,getOpacity(COLOR)); 
	Stroke(255,20,20,getOpacity(OUTLINECOLOR));
//	TextEnd(getWidth(94), getHeight(80), "ᐛ", osdicons, text_scale*2.3);	
	Fill(COLOR);
    Stroke(OUTLINECOLOR);
    Fill(COLOR);
    Stroke(OUTLINECOLOR);
...
void draw_EFFIC(int armed, int gpsspeed, float current, float pos_x, float pos_y, float scale){
    ...
    if(armed==1){
        if (gpsspeed>20)
        {
        EFF=current/(gpsspeed)*1000;//___________________test speed 70___________________!
        }
        else
        {
         EFF=120;
    ...
    Fill(COLOR); //normal
    Stroke(OUTLINECOLOR);
    Text(getWidth(pos_x), getHeight(pos_y), "ʘ", osdicons, text_scale); //................eff icon

    sprintf(buffer, "%.1f", EFF);
...
void draw_VARIO(float climb, float pos_x, float pos_y, float scale){
    char* ff;
    float text_scale = getWidth(2);
    VGfloat width_value = TextWidth("-00.0", myfont, text_scale);
    Fill(COLOR); //normal
    Stroke(OUTLINECOLOR);
    TextEnd(getWidth(pos_x), getHeight(pos_y), "ʆ", osdicons, text_scale); //................Vario main icon

    //NUM VALUE
    Fill(255,130,0,1); //normal
    if (climb > 0.0f) {sprintf(buffer, "+%.1f", climb);}
    else {sprintf(buffer, "%.1f", climb);}//...............................TEST ff!
    Text(getWidth(pos_x), getHeight(pos_y-1), buffer, myfont, text_scale*0.7);
    //NUM VALUE

    Fill(0,208,0,1); //normal
    Stroke(OUTLINECOLOR);
    TextEnd(getWidth(pos_x), getHeight(pos_y), "ʇ", osdicons, text_scale); //................Zero slsh icon

    Fill(0,0,240,1); //normal
    Stroke(OUTLINECOLOR);
    TextEnd(getWidth(pos_x), getHeight(pos_y), "ʈ", osdicons, text_scale); //................Zero arrow icon
    
    if (climb>=0)
     {
     if      (climb <= 0.5 ){ff = "ɠ";}//0
     else if (climb > 0.5 && climb <= 0.8 )  {ff = "ɡ";}//0.5
     else if (climb > 0.8 && climb <= 1.2 )  {ff = "ɢ";}//1
     else if (climb > 1.2 && climb <= 1.7 )  {ff = "ɣ";}//1.5
     else if (climb > 1.7 && climb <= 2.2 )  {ff = "ɤ";}//2
     else if (climb > 2.2 && climb <= 2.6 )  {ff = "ɥ";}//2.5
     else if (climb > 2.6 && climb <= 3.2 )  {ff = "ɦ";}//3
     else if (climb > 3.2 && climb <= 3.6 )  {ff = "ɧ";}//3.5
     else if (climb > 3.5)   {ff = "ɨ";}//4
     else{ff="ɠ";}//0
     }
    ...
    if (climb<0)
     {
     if      (climb >= -0.5 ){ff = "ɠ";}//0
     else if (climb < -0.5 && climb >= -0.8 )  {ff = "Ⱥ";}//0.5
     else if (climb < -0.8 && climb >= -1.2 )  {ff = "Ȼ";}//1
     else if (climb < -1.2 && climb >= -1.7 )  {ff = "ȼ";}//1.5
     else if (climb < -1.7 && climb >= -2.2 )  {ff = "Ƚ";}//2
     else if (climb < -2.2 && climb >= -2.6 )  {ff = "Ⱦ";}//2.5
     else if (climb < -2.6 && climb >= -3.2 )  {ff = "ȿ";}//3
     else if (climb < -3.2 && climb >= -3.6 )  {ff = "ɀ";}//3.5
     else if (climb < -3.5)   {ff = "Ɂ";}//4
     //else{ff="ɠ";}//0
     }
   
    Fill(250,0,0,1); //normal
    Stroke(OUTLINECOLOR);
    TextEnd(getWidth(pos_x), getHeight(pos_y), ff, osdicons, text_scale); //................arrow pionter icon
...
void draw_SIDE_THR(uint16_t throttle, float pos_x, float pos_y, float scale){
    ...
    Fill(COLOR); //normal
    Stroke(OUTLINECOLOR);
    Text(getWidth(pos_x), getHeight(pos_y), "Ȝ", osdicons, text_scale); //....THROTTLE main icon
    sprintf(buffer, "%03d", throttle);//......................................Numeric Value
    Text(getWidth(pos_x), getHeight(pos_y-1), buffer, myfont, text_scale*0.7);
    Fill(10,250,10,1); //Green
    Text(getWidth(pos_x), getHeight(pos_y), "ȝ", osdicons, text_scale); //....
    Text(getWidth(pos_x), getHeight(pos_y), "Ȟ", osdicons, text_scale); //....
    if(tthr<= 6 ){thrv = "Ƞ";}
    else if (tthr > 6 && tthr <= 14 ){thrv = "ȡ";}
    else if (tthr > 14 && tthr <= 24 ){thrv = "Ȣ";}
    else if (tthr > 24 && tthr <= 34 ){thrv = "ȣ";}
    else if (tthr > 34 && tthr <= 44 ){thrv = "Ȥ";}
    else if (tthr > 44 && tthr <= 54 ){thrv = "ȥ";}
    else if (tthr > 54 && tthr <= 64 ){thrv = "Ȧ";}
    else if (tthr > 64 && tthr <= 74 ){thrv = "ȧ";}
    else if (tthr > 74 && tthr <= 84 ){thrv = "Ȩ";}
    else if (tthr > 84 && tthr <= 94 ){thrv = "ȩ";}
    else if (tthr > 94 && tthr <= 110){thrv = "Ȫ";}
    else{thrv = "Ƞ";}
    
    Fill(250,0,0,1); //normal
    Text(getWidth(pos_x), getHeight(pos_y), thrv, osdicons, text_scale); //................Draw_ARROW
    
 }
void draw_ARMSTAT(int armed, float pos_x, float pos_y, float scale){
    float text_scale = getWidth(2) * scale;
    VGfloat width_value = TextWidth("0", myfont, text_scale);
    Fill(255,255,255,1); //normal
    Stroke(OUTLINECOLOR);
    //TextEnd(getWidth(pos_x), getHeight(pos_y), "ʖ", osdicons, text_scale);//....status frame
    if(armed==1){
        if(stat_arm_var<50)
        {
        TextEnd(getWidth(pos_x), getHeight(pos_y), "ʖ", osdicons, text_scale);//....status frame
        Fill(0,240,0,0.3); //normal
        Stroke(OUTLINECOLOR);
        TextEnd(getWidth(pos_x), getHeight(pos_y), "ʕ", osdicons, text_scale);//ʕ
        stat_arm_var++;
        }
    ...
    if(armed!=1){
    TextEnd(getWidth(pos_x), getHeight(pos_y), "ʖ", osdicons, text_scale);//....status frame
    Fill(240,0,0,1); //normal
    Stroke(OUTLINECOLOR);
    TextEnd(getWidth(pos_x), getHeight(pos_y), "ʗ", osdicons, text_scale);//ʗ
    }
 }
void draw_CELL_V(float voltage, int cells, float warn, float crit, float pos_x, float pos_y, float scale){
    float cell_v=voltage/cells;
    Stroke(OUTLINECOLOR);
    float text_scale = getWidth(2) * scale;
    VGfloat width_value = TextWidth("000", myfont, text_scale);
    Fill(COLOR);
...
void draw_TOTAL_AMPS(float current, float pos_x, float pos_y, float scale){ 
    ...
    Fill(COLOR);
    float text_scale = getWidth(2) * scale;
    Text(getWidth(pos_x), getHeight(pos_y), "ɲ", osdicons, text_scale );//Volt
    sprintf(buffer, "%04.0f", current);
    Text(getWidth(pos_x), getHeight(pos_y), buffer, myfont, text_scale);
    Fill(20,250,20,1);
    Text(getWidth(pos_x), getHeight(pos_y), "ɱ", osdicons, text_scale );//Volt
 }
//-------------------------------------------------------------------------------------------------------------------------------------------nwo
...
void draw_TOTAL_DIST(int gpsspeed, float pos_x, float pos_y, float scale){
    ...
    float text_scale = getWidth(2) * scale;
    VGfloat height_text = TextHeight(myfont, text_scale)+getHeight(0.3)*scale;
    Text(getWidth(pos_x), getHeight(pos_y), "ʔ", osdicons, text_scale*0.7);//ʗ
    sprintf(buffer, "%05.1f", total_dist);
    Text(getWidth(pos_x), getHeight(pos_y), buffer, myfont, text_scale);
    //Text(getWidth(pos_x+5), getHeight(pos_y), " km", myfont, text_scale*0.6);
...
void draw_mslalt(float mslalt, float pos_x, float pos_y, float scale){
    float text_scale = getWidth(2) * scale;
    curr_c_alt=mslalt-start_point_alt;
    Fill(COLOR); //normal
    Stroke(OUTLINECOLOR);
...
void draw_card_signal(int8_t signal, int signal_good, int card, int adapter_cnt, int restart_count, int packets, int wrongcrcs, int type, int totalpackets, int totalpacketslost, float pos_x, float pos_y, float scale){
    ...
    TextEnd(getWidth(pos_x), getHeight(pos_y) - card * height_text, buffer, myfont, text_scale*0.7);
    Fill(COLOR);
    Stroke(OUTLINECOLOR);
